...
A = imread('Original_images/Original_Image.jpg');
I = rgb2gray(A);
...
for k=1:idx
    ...
    saveas(gcf,['output/worms/image_',num2str(k),'.png']);
    hold on;
...
%     [line_points,points_unused] = form_line(points,25);
...
%     hold on;
    ...
    [num_points,~] = size(line_points);
    [num_unused,~] = size(points_unused);
    show_line(line_points);
    hold on;
    plot(points_unused(:,2),points_unused(:,1),'*r');
    hold off;
    saveas(gcf,['output/worms/line_',num2str(k),'.png']);
    save (['output/worms/data_',num2str(k),'.mat']);
    if num_unused == 0
        save (['output/good_worms/data_',num2str(i),'.mat']);
        imshow(worm,'InitialMagnification','fit');title(i);
        saveas(gcf,['output/good_worms/image_',num2str(i),'.png']);
        hold on;
        plot(edge_sample(:,2),edge_sample(:,1),'xm');
        plot(mid_points(:,2),mid_points(:,1),'.k'); % link the original mid_points
        plot(mid_points(:,2),mid_points(:,1),'ob');
        ...
        plot(points_unused(:,2),points_unused(:,1),'*r');
        hold off;
        saveas(gcf,['output/good_worms/line_',num2str(i),'.png']);
        i = i+1;
    else
        save (['output/bad_worms/data_',num2str(j),'.mat']);
        imshow(worm,'InitialMagnification','fit');title(j);
        saveas(gcf,['output/bad_worms/image_',num2str(j),'.png']);
        hold on;
        plot(edge_sample(:,2),edge_sample(:,1),'xm');
        plot(mid_points(:,2),mid_points(:,1),'.k'); % link the original mid_points
        plot(mid_points(:,2),mid_points(:,1),'ob');
        ...
        plot(points_unused(:,2),points_unused(:,1),'*r');
        hold off;
        saveas(gcf,['output/bad_worms/line_',num2str(j),'.png']);
        j = j+1;
    end
%     pause(2);
end
