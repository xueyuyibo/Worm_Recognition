...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function points = mid_points(worm,edge_sample)
...
%     plot(edge_sample(i,2),edge_sample(i,1),'or');
    ...
    for j=1:num-group_num % each group has 3 points, this loop is to find the closest goal_points
...
global I;
[r,~] = size(I);
y = mod(vector,r)+(mod(vector,1440)==0)*r;
x = (vector-y)/r+1;
cor_area = [x,y];
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [line_points,points_unused] = form_line(points,dis,ang_diff)
            ...
            if i==1
                ...
                if i>=3
                    if line_points(i,1) ~=line_points(i-1,1)
                        ang_now = atan((line_points(i,2)-line_points(i-1,2))/(line_points(i,1)-line_points(i-1,1)))/pi*180;
                    else
                        ang_now = 90;
                    end
                    if abs(ang_now-ang_before)<=ang_diff
                        distance = norm(line_points(i,:)-points2(j,:));
                        n = j;flag = 0;
                    end
                    if line_points(i,1) ~=line_points(i-1,1)
                        ang_before = atan((line_points(i,2)-line_points(i-1,2))/(line_points(i,1)-line_points(i-1,1)))/pi*180;
                    else
                        ang_before = 90;
                    end
                else
